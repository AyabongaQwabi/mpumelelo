...
var createElement = (() => {
  ...
  if (/*@cc_on@*/ /*@if (@_win32)!/*@end@*/ false) {
    ...
    var setAttribute = (element, attr, value) => {
...
function getParameter(parameterName) {
  ...
  var parameterNameEquals = parameterName + "="
  var queryString = window.location.href.slice(window.location.href.indexOf("?") + 1)
  if (queryString.length > 0) {
    // Find the beginning of the string
    var begin = queryString.indexOf(parameterNameEquals)
    // If the parameter name is not found, skip it, otherwise return the value
    if (begin != -1) {
      // Add the length (integer) to the beginning
      begin += parameterNameEquals.length
...
export function postPaymentToPayFast(
  payFastUrl,
  merchantId,
  merchantKey,
  returnUrl,
...
export default postPaymentToPayFast
